...
func nftForward(rules string, target string, option string) error {
	...
	// nft list ruleset
	// Create table: nft add table inet hui_hysteria_porthopping
	// Create chain: nft add chain inet hui_hysteria_porthopping prerouting { type nat hook prerouting priority dstnat\; policy accept\; }
	// Add rule: nft add rule inet hui_hysteria_porthopping prerouting iifname enp1s0 udp dport {30000-40000} counter redirect to :444 comment hui_hysteria_porthopping
