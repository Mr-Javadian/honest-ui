      ...
      <div class="settings-grid">
          ...
          <div class="card-body">
            <div class="form-row three-col">
              <el-form-item :label="$t('config.huiWebPort') || 'Web Panel Port'" prop="huiWebPort">
                <el-input v-model="dataForm.huiWebPort" placeholder="e.g., 8081" clearable />
              </el-form-item>
              <el-form-item :label="$t('config.huiWebContext') || 'Web Root Context Path'" prop="huiWebContext">
                <el-input v-model="dataForm.huiWebContext" placeholder="e.g., /hui or /admin" clearable />
              </el-form-item>
              <el-form-item :label="$t('config.timezone') || 'System Timezone'" prop="timeZone">
                <el-select v-model="dataForm.timeZone" filterable clearable :placeholder="$t('config.selectTimezone') || 'Select system timezone'">
        ...
        <div class="settings-card">
          ...
          <div class="card-body">
            <div class="form-row two-col">
              <el-form-item :label="$t('config.hysteria2TrafficTime') || 'Traffic Sync Interval (seconds)'" prop="hysteria2TrafficTime">
                <el-input v-model="dataForm.hysteria2TrafficTime" placeholder="e.g., 1" clearable />
              </el-form-item>
              <el-form-item :label="$t('config.resetTrafficCron') || 'Traffic Auto-Reset Schedule'" prop="resetTrafficCron">
                <el-tooltip :content="$t('config.resetTrafficCronTip') || 'Select schedule: Daily, Weekly, Monthly, or Never'" placement="bottom" :teleported="false">
                  <el-select v-model="dataForm.resetTrafficCron" clearable :placeholder="$t('config.resetTrafficCron') || 'Select schedule'">
                    <el-option v-for="item in cronResetTraffic" :key="item.value" :label="item.key" :value="item.value" />
                  </el-select>
                </el-tooltip>
              </el-form-item>
            </div>
        ...
        <div class="settings-card">
          ...
          <div class="card-body">
              ...
              <el-form-item :label="$t('config.huiHttps') || 'Enable HTTPS for Web Panel'" prop="huiHttps">
                <div class="https-toggle">
                  <el-select v-model="huiHttps" ref="huiHttpsRef" style="width:200px">
                    <el-option v-for="item in huiHttpsList" :key="item.key" :label="item.key" :value="item.value" />
                  </el-select>
            ...
            <div v-if="huiHttps" class="form-row two-col" style="margin-top:16px">
              <el-form-item :label="$t('config.huiCrtPath')" prop="huiCrtPath">
                <div class="input-with-action">
                  <el-input v-model="dataForm.huiCrtPath" :placeholder="$t('config.huiCrtPath')" clearable />
              ...
              <el-form-item :label="$t('config.huiKeyPath')" prop="huiKeyPath">
                <div class="input-with-action">
                  <el-input v-model="dataForm.huiKeyPath" :placeholder="$t('config.huiKeyPath')" clearable />
              ...
              <div class="preference-item">
                ...
                <div class="theme-group">
                  <button class="theme-btn" :class="{ active: theme === 'light' }" @click="theme !== 'light' && cycleTheme()">
                    <i-ep-sunny /><span class="theme-btn-label">Light Mode</span>
                  </button>
                  <button class="theme-btn" :class="{ active: theme === 'dark' }" @click="theme !== 'dark' && cycleTheme()">
                    <i-ep-moon /><span class="theme-btn-label">Dark Mode</span>
                  </button>
                  <button class="theme-btn" :class="{ active: theme === 'ultra-dark' }" @click="theme !== 'ultra-dark' && cycleTheme()">
                    <i-ep-star-filled /><span class="theme-btn-label">High Contrast</span>
                  </button>
                </div>
              </div>
              <div class="preference-item">
...
const dataFormRules = {
  huiWebPort: [
    ...
    { pattern: /^\d+$/, message: "field must be a integer", trigger: ["change", "blur"] },
  ],
  huiWebContext: [
    { required: true, message: "Required", trigger: ["change", "blur"] },
    { pattern: /^\/([a-z0-9]+(\/[a-z0-9]+)*)?$/, message: "field must start with / and contain only lowercase letters (a-z) and numbers (0-9)", trigger: ["change", "blur"] },
  ],
  hysteria2TrafficTime: [
    { required: true, message: "Required", trigger: ["change", "blur"] },
    { pattern: /^\d+(\.\d)?$/, message: "field must be a number with up to one decimal place", trigger: ["change", "blur"] },
  ],
};
...
const { dataForm, huiHttps, fileList, crtFileList, keyFileList } = toRefs(state);
  ...
  dataFormRef.value.validate((valid: boolean) => {
    if (valid) {
      ...
      let configs: ConfigsUpdateDto[] = [
        { key: huiWebPortKey, value: state.dataForm.huiWebPort },
        { key: huiWebContext, value: state.dataForm.huiWebContext },
        { key: hysteria2TrafficTimeKey, value: state.dataForm.hysteria2TrafficTime },
        { key: huiCrtPathKey, value: state.dataForm.huiCrtPath },
        { key: huiKeyPathKey, value: state.dataForm.huiKeyPath },
        { key: resetTrafficCronKey, value: state.dataForm.resetTrafficCron },
        { key: timeZoneKey, value: state.dataForm.timeZone },
      ];
...
const setConfig = async () => {
  const { data } = await listConfigApi({
    keys: [huiCrtPathKey, huiWebContext, huiKeyPathKey, huiWebPortKey, hysteria2TrafficTimeKey, resetTrafficCronKey, timeZoneKey],
  });
  data.forEach((configVo) => {
    if (configVo.key === huiWebPortKey) state.dataForm.huiWebPort = configVo.value;
    else if (configVo.key === huiWebContext) state.dataForm.huiWebContext = configVo.value;
    else if (configVo.key === hysteria2TrafficTimeKey) state.dataForm.hysteria2TrafficTime = configVo.value;
    else if (configVo.key === huiCrtPathKey) state.dataForm.huiCrtPath = configVo.value;
    else if (configVo.key === huiKeyPathKey) state.dataForm.huiKeyPath = configVo.value;
    else if (configVo.key === resetTrafficCronKey) state.dataForm.resetTrafficCron = configVo.value;
    else if (configVo.key === timeZoneKey) state.dataForm.timeZone = configVo.value;
  });
  if (state.dataForm.huiCrtPath != "" && state.dataForm.huiKeyPath != "") {
    state.huiHttps = 1;
  }
...
const handleImport = async (params: UploadRequestOptions) => {
  if (state.fileList.length > 0) {
    ...
    finally { await setConfig(); }
  }
};

const beforeImport = (file: UploadRawFile) => {
  if (!file.name.endsWith(".json")) { ElMessage.error("file format not supported"); return false; }
  if (file.size / 1024 / 1024 > 2) { ElMessage.error("the file is too big, less than 2 MB"); return false; }
};
...
const setCertPath = async () => {
  try {
    ...
    state.dataForm.huiCrtPath = data.crtPath;
    state.dataForm.huiKeyPath = data.keyPath;
  } catch (e) {}
};

const uploadCertFile = async (params: UploadRequestOptions) => {
  try {
    if (!params.file.name.endsWith(".crt") && !params.file.name.endsWith(".key")) { ElMessage.error("file format not supported"); }
    if (params.file.size > 1024 * 1024) { ElMessage.error("the file is too big"); }
    let formData = new FormData();
    formData.append("file", params.file);
    const { data } = await uploadCertFileApi(formData);
    if (params.file.name.endsWith(".crt")) state.dataForm.huiCrtPath = data;
    else if (params.file.name.endsWith(".key")) state.dataForm.huiKeyPath = data;
  } catch (e) {}
};
...
.settings-card {
  ...
  .card-accent {
    ...
    &.accent-purple { background: linear-gradient(90deg, #a78bfa, #c084fc); }
    &.accent-green { background: linear-gradient(90deg, #34d399, #10b981); }
    &.accent-orange { background: linear-gradient(90deg, #fbbf24, #f59e0b); }
  }
}
...
.card-icon {
  ...
  flex-shrink: 0;
  &.icon-purple { color: #a78bfa; background: rgba(167,139,250,0.12); }
  &.icon-green { color: #34d399; background: rgba(52,211,153,0.12); }
  &.icon-orange { color: #f59e0b; background: rgba(245,158,11,0.12); }
}

.card-title { margin: 0 0 2px; font-size: 15px; font-weight: 600; color: var(--el-text-color-primary); }
.card-desc { margin: 0; font-size: 12px; color: var(--el-text-color-secondary); line-height: 1.4; }
...
.https-toggle { display: flex; gap: 12px; align-items: center; }
.input-with-action { display: flex; gap: 8px; align-items: center; .el-input { flex: 1; } }
...
.pref-label { font-size: 14px; font-weight: 500; color: var(--el-text-color-primary); }
.pref-hint { font-size: 12px; color: var(--el-text-color-placeholder); }

.pref-control {
  :deep(.el-input__wrapper) { background: transparent; border: none; padding: 0; box-shadow: none !important; }
  :deep(.el-input__inner) { font-size: 13px; color: var(--el-text-color-regular); }
}
...
.theme-btn {
  ...
  padding: 7px 12px; border-radius: 10px; border: 1px solid var(--el-border-color-light);
  cursor: pointer; background: var(--el-fill-color-light); color: var(--el-text-color-secondary);
  font-size: 16px; transition: all 0.2s ease; min-width: 58px;
  &:hover { background: var(--el-fill-color); color: var(--el-color-primary); border-color: var(--el-color-primary); }
  &.active { border-color: var(--el-color-primary); color: var(--el-color-primary); background: color-mix(in srgb, var(--el-color-primary) 12%, transparent); box-shadow: 0 0 0 1px var(--el-color-primary); }
}

.theme-btn-label { font-size: 9px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.3px; white-space: nowrap; }
...
.color-swatch {
  ...
  &.active { border-color: var(--el-text-color-primary); box-shadow: 0 0 0 2px var(--el-bg-color), 0 0 0 4px currentColor; }
}
...
@media (max-width: 768px) {
  ...
  .preference-item { flex-direction: column; align-items: flex-start; gap: 10px; }
